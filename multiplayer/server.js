...
io.sockets.on("connection", function (socket) {
	...
	socket.on("createRoom", function(name) {
		if (people[socket.id].inroom) {
			...
			console.log( people[socket.id].name +" created room: " +name + " connected with id " + id);
	...
	socket.on("startgame", function(id) {
		var usrname = people[socket.id].name;
		var rmnames = rooms[people[socket.id].inroom].name;
		socket.emit("update", usrname + " is starting a game for room " + rmnames);
        //console.log(io.sockets.in(socket.room)
		console.log(people[socket.id].name + " is starting a game for room " + rooms[people[socket.id].inroom].name);
	});

	socket.on("leaveRoom", function(id) {
		var room = rooms[id];
		console.log(people[socket.id].name + "left the room: " + rooms[people[socket.id].inroom].name );
		if (room)
